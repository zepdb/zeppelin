...
/// A bitmap-resolved filter must return the same rows as the exact-attribute
/// filter on every scan path, not just the quantized ones.
///
/// `QuantizationType::None` is the regression case: the flat scan has no rerank
/// phase to re-read attributes, so eliding coarse attrs without also skipping
/// the final post-filter silently returns zero rows.
#[tokio::test]
async fn bitmap_filter_elides_only_resolved_coarse_attrs() {
    for quantization in [QuantizationType::None, QuantizationType::Scalar] {
        let harness = TestHarness::new().await;
        let (store, counter) = counting_store(&harness.store);
        let vectors = ivf_vectors();
        let mut bitmap_config = ivf_config(quantization);
        bitmap_config.bitmap_index = true;
        let bitmap_index = IvfFlatIndex::build(
            &vectors,
            &bitmap_config,
            &store,
            &harness.artifact_origin_namespace(&format!("bitmap-filter-{quantization:?}")),
            "seg_bitmap_filter",
        )
        .await
        .unwrap();
        let exact_index = IvfFlatIndex::build(
            &vectors,
            &ivf_config(quantization),
            &store,
            &harness.artifact_origin_namespace(&format!("exact-filter-{quantization:?}")),
            "seg_exact_filter",
        )
        .await
        .unwrap();
        ...
        for (case, filter, bitmap_only) in cases {
            let label = format!("{case} ({quantization:?})");
            counter.reset();
            ...
            let exact_id_scores: Vec<_> = exact_results
                ...
                .collect();
            assert!(
                !exact_id_scores.is_empty(),
                "{label} fixture must match at least one row"
            );
            ...
            if bitmap_only {
                assert!(
                    ...
                    "{label} must retain final attrs enrichment while removing coarse attrs GETs"
                );
            } else {
                assert_eq!(
