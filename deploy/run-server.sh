...
docker run -d --name zeppelin \
    ...
    -e RUST_LOG=info \
    -e ZEPPELIN_MAX_BATCH_SIZE="${ZEPPELIN_MAX_BATCH_SIZE:-50000}" \
    -e ZEPPELIN_MAX_WAL_FRAGMENTS="${ZEPPELIN_MAX_WAL_FRAGMENTS:-1}" \
    -e ZEPPELIN_COMPACTION_INTERVAL_SECS="${ZEPPELIN_COMPACTION_INTERVAL_SECS:-10}" \
    -e ZEPPELIN_DEFAULT_NUM_CENTROIDS="${ZEPPELIN_DEFAULT_NUM_CENTROIDS:-1024}" \
    -e ZEPPELIN_DEFAULT_NPROBE="${ZEPPELIN_DEFAULT_NPROBE:-8}" \
    ...
    -e ZEPPELIN_MEMORY_CACHE_MAX_MB="${ZEPPELIN_MEMORY_CACHE_MAX_MB:-8192}" \
    -e ZEPPELIN_FTS_INDEX="${ZEPPELIN_FTS_INDEX:-false}" \
    -e ZEPPELIN_HIERARCHICAL="${ZEPPELIN_HIERARCHICAL:-true}" \
