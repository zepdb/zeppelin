...
//!
//! The object stored in the configured object store is authoritative. Memory and disk may cache
...
- `# Side Effects`: Describe object-storage requests, artifact creation, manifest
...
/// # Consistency
///
/// The object-storage manifest remains authoritative. Cache state cannot satisfy or
...
- Use a normal `rust` fence for self-contained code that rustdoc should compile
  and test.
- Use `no_run` when the code should compile but requires an object-storage
  service, a network, a Tokio runtime setup, or other external state to execute.
...
- memory to disk to object-storage cache lookup;
...
- **Fail loudly.** Do not describe a fallback or default unless the code and
  ...
  degradation.
- **The configured object store is authoritative.** Memory and local disk are
  disposable caches. Higher layers access object storage through
  `src/storage/`.
...
- Retrieval: vector search, bitmap prefiltering, BM25, hybrid score fusion, and
  result merging.
- Cache: memory/disk/object-storage lookup order, stale versus authoritative state, pinned
...
///   is a storage key, not an HTTP namespace path.
///
/// # Returns
///
...
```rust
// Drop the guard before awaiting object storage so unrelated cache reads can continue.
drop(cache_guard);
...
- integration tests use the shared `TestHarness` and name their required
  backend explicitly; the default is the `object_store` in-memory backend,
  while CAS-, concurrency-, and substrate-specific gates use MinIO, the patched
  GCS emulator, or Azurite;
...
- assertions should explain the authoritative object-storage state they verify.
