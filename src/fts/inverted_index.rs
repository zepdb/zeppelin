...
/// Orders segment-wide hits by score, cluster, and cluster-local position.
///
/// # Parameters
///
/// - `a`: First `(cluster index, position, score)` hit.
/// - `b`: Second hit in the same representation.
///
/// # Returns
///
/// Descending-score order with ascending cluster and position tie-breakers. A
/// tie between `(1, 8, 2.5)` and `(3, 0, 2.5)` therefore favors cluster 1.
...
impl FtsSegmentMeta {
    /// Aggregates cluster-local indexes into segment-wide BM25 statistics.
    ///
    /// Document counts and term document frequencies are summed. Average field
    /// length is weighted by each cluster's indexed-document count, so a tiny
    /// cluster does not influence the mean as much as a large one.
    ///
    /// # Parameters
    ///
    /// - `cluster_indexes`: Cluster-local indexes that belong to one logical
    ///   segment. Their slice order does not affect the aggregate statistics.
    /// - `fts_field_names`: Field names to summarize. The returned `fields`
    ///   vector preserves this order; repeated names overwrite the same map
    ///   entry but remain repeated in that vector.
    ///
    /// # Returns
    ///
    /// Owned metadata with an entry for every requested field, including fields
    /// with zero indexed documents. Such fields have zero average length and an
    /// empty term-frequency map.
    ///
    /// # Performance
    ///
    /// Visits every posting-list header for every requested field and clones
    /// each distinct term into the aggregate map. It does not visit individual
    /// postings or perform object-store I/O.
    ///
    /// # Examples
    ///
    /// If one cluster has 2 indexed `title` documents averaging 3 tokens and a
    /// second has 1 averaging 9, the segment average is `(2*3 + 1*9) / 3 = 5`,
    /// not the unweighted mean 6.
    ///
    /// # Rust Notes for Java/C Engineers
    ///
    /// The input slice is borrowed while all returned maps and strings are
    /// owned. `BTreeMap::entry` performs insert-or-update without a second
    /// lookup, similar to Java's `Map.compute` but statically restricted by one
    /// mutable borrow. C would require explicit tree ownership and cleanup.
    ...
    /// Scores cluster-local postings with segment-wide IDF and length statistics.
    ///
    /// This operation combines the exact document lengths recoverable from each
    /// [`InvertedIndex`] with this metadata's segment-wide corpus statistics.
    /// Matching uses OR semantics and sums query-token contributions per
    /// document.
    ///
    /// # Parameters
    ///
    /// - `field`: Field whose aggregate statistics and cluster postings to use.
    /// - `query_tokens`: Already normalized exact tokens. Repeated tokens are
    ///   scored repeatedly.
    /// - `params`: BM25 field parameters.
    /// - `cluster_indexes`: `(logical cluster index, index)` pairs to search.
    ///   Cluster indexes need not be contiguous, but each index must correspond
    ///   to the statistics from which this metadata was built.
    ///
    /// # Returns
    ///
    /// Owned `(cluster index, position, score)` triples for every matching
    /// document, sorted by descending score, ascending cluster, then ascending
    /// position. A missing field or no known query terms returns an empty vector.
    ///
    /// # Performance
    ///
    /// Reconstructs document lengths for every supplied cluster containing the
    /// field, visits postings for matching terms, and sorts all hits. All
    /// indexes must already be resident; loading N artifacts is a caller cost.
    ///
    /// # Examples
    ///
    /// If `rare` appears once across two clusters, both clusters use the same
    /// segment-wide IDF for that term. A cluster omitted from `cluster_indexes`
    /// contributes no results even if it was included in these statistics.
    ///
    /// # Rust Notes for Java/C Engineers
    ///
    /// Iterating `&[(usize, InvertedIndex)]` borrows each owned index; scoring
    /// cannot move or mutate it. The returned triples contain only copied
    /// integers and floats, so their lifetime is independent of the input.
