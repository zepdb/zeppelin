...
mod tests {
    ...
    use super::{
        anchor_key, encode_writer_head, new_writer_head, read_writer_head, AuditRecord,
        AuditRuntime, AuditSinkError,
    };
    ...
    impl ObjectSigner for TestSigner {
        fn signer_node(&self) -> &str {
            "zsn1_audit_writer_head_test"
        }

        fn sign(&self, message: &[u8]) -> Vec<u8> {
            message.iter().copied().take(64).collect()
        }
    ...
    fn signed_store() -> ZeppelinStore {
        let store = ZeppelinStore::new(Arc::new(InMemory::new()));
        store
            .install_object_signer(Arc::new(TestSigner {
                store: store.clone(),
            }))
            .unwrap_or_else(|error| panic!("test signer must install: {error}"));
        store
    }

    async fn expire_writer_lease(store: &ZeppelinStore, signer: &str) {
        let mut loaded = read_writer_head(store, signer)
            .await
            .unwrap_or_else(|error| panic!("writer head must load: {error}"))
            .unwrap_or_else(|| panic!("writer head must exist"));
        loaded.document.lease_expires_at = Some(Utc::now() - ChronoDuration::seconds(1));
        let outcome = store
            .put_if_match_outcome(
                &loaded.key,
                encode_writer_head(&loaded.document)
                    .unwrap_or_else(|error| panic!("writer head must encode: {error}")),
                &loaded.e_tag,
            )
            .await
            .unwrap_or_else(|error| panic!("writer lease must expire: {error}"));
        assert!(matches!(
            outcome,
            crate::storage::ConditionalPutOutcome::Updated { .. }
        ));
    }
    ...
    #[tokio::test]
    async fn client_health_detects_an_aborted_writer_task() {
        let store = signed_store();
        let (client, mut runtime) =
            AuditRuntime::start(store, "aborted-writer", Duration::from_secs(60))
                .await
                .unwrap_or_else(|error| panic!("abort fixture must start: {error}"));
        assert!(client.is_healthy());
        let task = runtime
            .task
            .take()
            .unwrap_or_else(|| panic!("durable runtime must own a writer task"));
        task.abort();
    ...
    #[tokio::test]
    async fn production_head_resumes_an_unanchored_crash_stream() {
        let store = signed_store();
        let (client, runtime) =
            AuditRuntime::start_for_published_signer(store.clone(), Duration::from_secs(60))
                .await
                .unwrap_or_else(|error| panic!("first production writer must start: {error}"));
        let stream_id = client.node_id().to_string();
        client
            .submit_durable(AuditRecord::open_unsafe_boot(Utc::now(), &stream_id))
            .await
            .unwrap_or_else(|error| panic!("pre-crash record must persist: {error}"));
        let concurrent =
            AuditRuntime::start_for_published_signer(store.clone(), Duration::from_secs(60)).await;
        assert!(matches!(
            concurrent,
        ...
        drop(runtime);
        drop(client);
        let signer = stream_id
            .split_once('.')
            .map(|(signer, _)| signer)
            .unwrap_or_else(|| panic!("production stream must contain signer prefix"));
        expire_writer_lease(&store, signer).await;

        let (recovered, recovered_runtime) =
            AuditRuntime::start_for_published_signer(store, Duration::from_secs(60))
                .await
                .unwrap_or_else(|error| panic!("crash stream must recover: {error}"));
        assert_eq!(recovered.node_id(), stream_id);
        recovered_runtime
            .shutdown()
            .await
            .unwrap_or_else(|error| panic!("recovered stream must anchor: {error}"));
    }

    #[tokio::test]
    async fn production_head_rotates_after_a_sealed_same_day_stream() {
        let store = signed_store();
        let (client, runtime) =
            AuditRuntime::start_for_published_signer(store.clone(), Duration::from_secs(60))
                .await
                .unwrap_or_else(|error| panic!("first production writer must start: {error}"));
        let sealed_stream = client.node_id().to_string();
        client
            .submit_durable(AuditRecord::open_unsafe_boot(Utc::now(), &sealed_stream))
            .await
            .unwrap_or_else(|error| panic!("record must persist: {error}"));
        runtime
            .shutdown()
            .await
            .unwrap_or_else(|error| panic!("first stream must seal: {error}"));

        let (next, next_runtime) =
            AuditRuntime::start_for_published_signer(store, Duration::from_secs(60))
                .await
                .unwrap_or_else(|error| panic!("next production writer must start: {error}"));
        assert_ne!(next.node_id(), sealed_stream);
        next_runtime
            .shutdown()
            .await
            .unwrap_or_else(|error| panic!("empty successor must stop: {error}"));
    }

    #[tokio::test]
    async fn production_head_rotates_after_an_empty_clean_shutdown() {
        let store = signed_store();
        let (client, runtime) =
            AuditRuntime::start_for_published_signer(store.clone(), Duration::from_secs(60))
                .await
                .unwrap_or_else(|error| panic!("empty production writer must start: {error}"));
        let sealed_stream = client.node_id().to_string();
        runtime
            .shutdown()
            .await
            .unwrap_or_else(|error| panic!("empty production stream must seal: {error}"));

        let (next, next_runtime) =
            AuditRuntime::start_for_published_signer(store, Duration::from_secs(60))
                .await
                .unwrap_or_else(|error| panic!("clean restart must rotate immediately: {error}"));
        assert_ne!(next.node_id(), sealed_stream);
        next_runtime
            .shutdown()
            .await
            .unwrap_or_else(|error| panic!("empty successor must seal: {error}"));
    }

    #[tokio::test]
    async fn production_head_recovers_and_seals_a_cross_midnight_crash() {
        let store = signed_store();
        let signer = store
            .object_signer_node()
            .unwrap_or_else(|| panic!("test signer identity must exist"));
        let yesterday = Utc::now().date_naive() - ChronoDuration::days(1);
        let head = new_writer_head(&signer, yesterday);
        store
            .put_create(
                &super::writer_head_key(&signer),
                encode_writer_head(&head)
                    .unwrap_or_else(|error| panic!("writer head must encode: {error}")),
            )
            .await
            .unwrap_or_else(|error| panic!("writer head must publish: {error}"));
        let stream_id = head.stream_id.clone();
        let (client, runtime) =
            AuditRuntime::start_for_published_signer(store.clone(), Duration::from_secs(60))
                .await
                .unwrap_or_else(|error| panic!("yesterday writer must start: {error}"));
        client
            .submit_durable(AuditRecord::open_unsafe_boot(
                yesterday
            ...
            .await
            .unwrap_or_else(|error| panic!("yesterday record must persist: {error}"));
        drop(runtime);
        drop(client);
        expire_writer_lease(&store, &signer).await;
        ...
        let (recovered, recovered_runtime) =
            AuditRuntime::start_for_published_signer(store.clone(), Duration::from_secs(60))
                .await
                .unwrap_or_else(|error| panic!("cross-midnight stream must recover: {error}"));
        assert_eq!(recovered.node_id(), stream_id);
        recovered
            .submit_durable(AuditRecord::open_unsafe_boot(Utc::now(), &stream_id))
            .await
            .unwrap_or_else(|error| panic!("today record must roll the stream: {error}"));
        recovered_runtime
            .shutdown()
            .await
            .unwrap_or_else(|error| panic!("today stream must seal: {error}"));

        assert!(store.get(&anchor_key(yesterday, &stream_id)).await.is_ok());
        assert!(store
            .get(&anchor_key(Utc::now().date_naive(), &stream_id))
            .await
            .is_ok());
    }
}
